...
ATM.spellThreat = {}
function ATM:LoadSpell(spellID, callback)
...
function ATM:LoadSpells()
	-- ATM:print("ATM:LoadSpells")

	for playerClass,playerData in pairs(self.playerMixins) do
		for spellKey,spellData in pairs(playerData.spells or {}) do
			for spellID,threat in pairs(spellData.ranks) do
				-- we don't care about super low spell ids so this is fine for abilties with no threat ({[1] = 1337})
				ATM.spellThreat[spellID] = threat

				ATM:LoadSpell(spellID, function()
					local name = GetSpellInfo(spellID)
					L[spellID] = name

					if spellKey ~= name then
						-- ATM:print("newKey", spellID, spellKey, name)
						playerData.spells[name] = spellData
					end
				end)
			end
		end
	...
	for enemyID,enemyData in pairs(self.NPCs) do
		if enemyData.spells then
			for spellKey,spellData in pairs(enemyData.spells) do
				for _,spellID in pairs(spellData.ids or {spellData.id}) do
					ATM:LoadSpell(spellID, function()
						local name = GetSpellInfo(spellID)
						L[spellID] = name
						...
						if spellKey ~= name then
							ATM:print("newKey", spellID, spellKey, name)
							enemyData.spells[name] = spellData
							-- enemyData.spells[spellKey] = nil
						end
					end)
				end
			end
...
function ATM:Taunt(...)
	...
	-- print("ATM:Taunt", subevent, timestamp)
...
function ATM:UnitDetailedThreatSituation(sourceUnit, destUnit)
	...
	if enemy and player then
		...
		if threatvalue == tankThreat then
			rawthreatpct = 100
			threatpct = 100
		elseif threatvalue <= 0.1 then
			rawthreatpct = 0
			threatpct = 0
		else
			pullMultiplier = (rawthreatpct or 100)/(threatpct or 100) > 1.2 and 1.3 or 1.1
			rawthreatpct = threatvalue / tankThreat * 100
			threatpct = rawthreatpct / pullMultiplier
		end
	end

	return isTanking, status, threatpct, rawthreatpct, (threatvalue and threatvalue >= 0) and threatvalue or nil, pullMultiplier
end
